...
class ImportPage {
    ...
    /**
     * AI-Native: Autonomously detect and execute the correct import flow
     * Supports: Get Reviews, Import Reviews, Import Post flows
     * Includes: 2 retry attempts with page reload between retries
     */
    async detectAndExecuteFlow(platform, url, maxRetries = 2) {
        console.log(`[AI-NATIVE] Starting autonomous import for: ${platform}`);
        let attempt = 0;

        while (attempt <= maxRetries) {
            try {
                attempt++;
                console.log(`Attempt ${attempt}/${maxRetries + 1} for ${platform}`);

                // Step 1: Enter URL
                await this.enterUrl(url, platform);
                await this.page.waitForTimeout(2000); // Allow UI to update

                // Step 2: Detect which button is visible
                const getReviewsBtn = this.page.locator('button:has-text("Get Reviews")').first();
                ...
                const isGetReviews = await getReviewsBtn.isVisible({ timeout: 3000 }).catch(() => false);
                const isImportReviews = await importReviewsBtn.isVisible({ timeout: 3000 }).catch(() => false);
                const isImportPost = await importPostBtn.isVisible({ timeout: 3000 }).catch(() => false);

                if (isGetReviews) {
                    console.log(`[AI-NATIVE] Detected "Get Reviews" flow for ${platform}`);
                    await this.clickGetReviewsButton();
                    await this.page.waitForTimeout(3000); // Wait for reviews to load
                    await this.clickSelectAllCheckbox();
                    await this.clickFinalImportButton();
                } else if (isImportPost) {
                    console.log(`[AI-NATIVE] Detected "Import Post" flow for ${platform}`);
                    await this.clickImportPostButton();
                } else if (isImportReviews) {
                    console.log(`[AI-NATIVE] Detected "Import Reviews" flow for ${platform}`);
                    await this.clickImportReviewsButton();
                } else {
                    throw new Error(`No recognizable import button found for ${platform}`);
                }

                // Step 3: Verify success
                await this.verifySuccessMessage();
                console.log(`[AI-NATIVE] ✓ Successfully imported from ${platform}`);
                return { success: true, platform, attempt };

            } catch (error) {
                console.error(`[AI-NATIVE] Attempt ${attempt} failed for ${platform}: ${error.message}`);

                if (attempt > maxRetries) {
                    console.error(`[AI-NATIVE] ✗ All retries exhausted for ${platform}`);
                    return { success: false, platform, error: error.message, attempts: attempt };
                }

                // Retry: reload page and re-select platform
                console.log(`[AI-NATIVE] Retrying ${platform}... (reloading page)`);
